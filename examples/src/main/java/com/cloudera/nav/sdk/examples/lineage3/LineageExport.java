 ...
 * The example calculates lineage downstream with a length of 5. It uses
 * zero as the value of lineageOptions which applies all the filters.
 */
...
public class LineageExport {
  ...
  private final static String LINEAGE_EXPORT_API_QUERY_PARAMS = "?direction=%s&length=%s&lineageOptions=%s%s";
  ...
  public static void main(String[] args) throws Exception {
    ...
    Preconditions.checkArgument("downstream".equals(args[2].toLowerCase())
        || "upstream".equals(args[3].toLowerCase()));
    Preconditions.checkArgument(Integer.parseInt(args[3]) > 0);
    Preconditions.checkArgument(Integer.parseInt(args[4]) >= 0);
  ...
  public void run() throws IOException {
    ...
    do {
      MetadataQuery mq = new MetadataQuery(query, 100, cursorMark);
  ...
  private void fetchAndWriteLineage(Set<String> entityIds, FileWriter fw,
    ...
    ArrayList entities = (ArrayList) mapper.readValue(stream, Object.class);
    ...
    while (it.hasNext()) {
      ...
      if (children != null) {
        for (Map.Entry<String, Object> entry : children.entrySet()) {
          ArrayList<String> childrenIds = (ArrayList<String>) entry.getValue();
  ...
  private String constructEntityIdSetAsQueryParam(Set<String> entityIds) throws UnsupportedEncodingException {
    ...
    for (String id : entityIds) {
      sb.append("&entityIds=" + URLEncoder.encode(id, charset));
    }
  ...
  private HttpURLConnection createHttpConnection(ClientConfig config, Set<String> entityIds)
    ...
    String queryParamString = String.format(LINEAGE_EXPORT_API_QUERY_PARAMS,
        ...
        constructEntityIdSetAsQueryParam(entityIds), charset); // entityIds request for lineage
